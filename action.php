...
$db 	= new Database;
$util   = new util;
// Handling add new user Ajax request
if (isset($_POST['add'])) {
	...
	if($db->insert($fname, $lname, $email, $phone)) {
		...
		echo $util->showMessage('danger', 'User is not inserted yet!');
	}
}

// Handel all Users AJAX REQUEST
if (isset($_GET['read'])) {
	$users = $db->read();
	$output = '';
...
// handling edit ajax request 
  if (isset($_GET['edit'])) {
    $id = $_GET['id'];
    $user = $db->readOne($id);
    echo json_encode($user);
  }
// handling updating user ajax request 
  if (isset($_POST['update'])) {
    ...
    if ($db->update($id, $fname, $lname, $email, $phone)) {
      ...
      echo $util->showMessage('danger', 'Something went wrong!. Please try again...');
    }}

// handling Deleting Users Ajax request 
    ...
    if (isset($_GET['delete'])) {
        ...
        if ($db->delete($id)) {
        	...
        	echo $util->showMessage('danger', 'user is not deleted Please try again...');
